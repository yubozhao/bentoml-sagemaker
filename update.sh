...
echo "Check for existing sagemaker endpoint"
python sagemaker/check_existing_deployment.py $ENDPOINT_NAME
...
echo "Update Sagemaker endpoint"
read -r ENDPOINT_ARN <<<$(aws sagemaker update-endpoint --endpoint-name $ENDPOINT_NAME --endpoint-config-name $ENDPOINT_CONFIG_NAME | python get_json_value_from_return_struct.py EndpointArn)
